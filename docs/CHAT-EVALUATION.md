...
## Offline answer scorecards

Capture manually requested answers into a private JSON array under ignored `data/`. Each record has `case`, `answer`, `supplied_source_ids`, optionally measured `latency_ms`, `first_text_ms`, `cost_usd`, and a `review` object with explicit boolean fields: `factual_support`, `correct_dates`, `missing_data_honesty`, `followup_continuity`, `motive_attribution`. Provider usage audit records now include completion and first-visible-text latency where measured.

Run `.venv/bin/python scripts/evaluate_chat_answers.py data/chat-answers.json --output data/chat-scorecards.json`. The script makes no requests. Unknown source IDs are listed; a recognized citation does not prove the claim it accompanies. Missing human-review fields produce pending acceptance and missing latency/cost remain unmeasured. Do not commit private captured answers.
