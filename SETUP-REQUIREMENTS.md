...
## Chat revision 2 deployment

Deploy the same repository revision to Vercel (`apps/web` root) and Render (root Dockerfile). The confirmed API is `https://selery-13rz.onrender.com`; set Vercel’s server-only `SELERY_API_URL` to this origin. Keep existing backend secrets, database persistence and $5 LLM settings. `uv sync --frozen` installs the calendar dependency; startup creates `conversation_charts` (migration 005). No new account/key is needed. Preserve one API instance. `/health` and web `/api/release` expose `chat_revision: "2"` and deployment commit metadata for read-only verification. Native clients need a rebuilt bundle; GitHub/web deployment alone does not distribute an iPhone build.
