...
Chat revision 2 adds immutable `conversation_charts` snapshots (migration 005), server-validated signal-bound creation, search/rename/explicit question-excerpt summaries, and question-matched XNYS market context. Python filters finalized known-as-of bars before calculations. Citation records retain exact supplied observations, feed/provider and availability metadata; legacy rows deserialize nullable defaults. Shared safe Markdown renders identically through platform-specific text components.

OpenAI SSE visible-text deltas persist as pending assistant rows at most four times/second. Authenticated client polling recovers them without re-dispatch. Generation is shielded from client disconnects, held in a task registry, canceled safely on API shutdown, and marked interrupted after restart. Terminal usage settles the existing reservation; unknown outcomes retain it. One API process is still required. Web `/api/release` and backend `/health` expose chat revision/commit metadata for deployment checks, without secrets.

Web authentication also supports discoverable platform passkeys through `/api/v1/auth/passkeys`: public status/login options and verification, authenticated registration/list/removal. Registration additionally requires fresh workspace-password proof. `webauthn` verifies signatures, exact configured origin, RP ID, challenge and required user verification. Two-minute ceremonies bind to an HttpOnly browser cookie and, for enrollment, the existing session. A shared lock serializes challenge pruning/consumption and enrollment limits; saved signature counters update atomically. Password recovery remains available.

Migration 004 adds `auth_credentials`, containing public keys, identifiers, RP scope, counters and user-provided names. Passkey sessions reference their credential so deleting a key invalidates its sessions. The Vercel proxy strips the bearer token from successful passkey-login JSON and forwards the HttpOnly session cookie. No biometric or private-key material is stored by Selery. Deploy one API process: pending ceremonies and throttles are process-local. The current hosting choice is Render for the API and Vercel for the web; Railway remains an alternative. See [setup](SETUP-REQUIREMENTS.md#web-passkeys-on-render).
